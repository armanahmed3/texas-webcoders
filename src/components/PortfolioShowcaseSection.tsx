    ...
    <section id="portfolio" className="py-8 sm:py-12 px-4 sm:px-6 lg:px-8 relative z-10 min-h-screen flex flex-col justify-center">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          >
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-xs font-medium transition-all duration-200 cursor-pointer flex items-center gap-1.5 ${
                      selectedCategory === cat
                        ? isNone || isBlack
                          ...
                          : 'bg-zinc-950 text-white shadow-md scale-105 font-bold'
                        : isNone || isBlack
                          ? 'bg-zinc-900 border border-zinc-700/80 text-zinc-300 hover:text-white hover:bg-zinc-800'
                          : 'bg-white/90 border border-zinc-300 text-zinc-700 hover:text-black hover:bg-white shadow-sm'
                    }`}
                  >
            ...
            {!isHomePage && selectedCategory === 'Graphic Designing' && (
              <motion.div
                ...
                className="flex flex-wrap justify-center gap-2 mt-4 p-2.5 rounded-2xl max-w-4xl mx-auto backdrop-blur-md border shadow-inner bg-zinc-950/60 border-zinc-800/80"
              >
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-xl text-xs font-semibold transition-all duration-200 cursor-pointer font-['Montserrat'] flex items-center gap-1 ${
                        isActive
                          ? 'bg-white text-black shadow-md scale-105 font-bold'
                          : 'bg-zinc-900/90 border border-zinc-700/80 text-zinc-300 hover:text-white hover:bg-zinc-800'
                      }`}
                    >
                      <span>{subCat}</span>
                    </button>
                  ...
                  return (
                    <motion.div
                      ...
                      className={`group relative overflow-hidden rounded-2xl border transition-all duration-300 flex flex-col cursor-pointer ${
                        isDarkBlue || isBlack
                          ? 'bg-zinc-950 border-zinc-800 hover:border-white/60 shadow-xl'
                          : 'bg-white border-zinc-200 hover:border-black shadow-md hover:shadow-2xl'
                      }`}
                    >
                      {/* Big Media Container - dynamically expanded for Pitch Deck to show full slide deck */}
                      <div className={`relative overflow-hidden bg-zinc-900 flex flex-col ${
                        isPitchDeck ? 'h-[520px] sm:h-[600px] lg:h-[680px] bg-zinc-950' : 'h-64 sm:h-72'
                      }`}>
                        
                        {/* Video Media Render */}
                        {isPitchDeck ? (
                          /* Full Image Showcase for Pitch Deck */
                          <div className="relative w-full h-full bg-zinc-950 flex items-center justify-center p-2.5 sm:p-3 overflow-hidden">
                            <img
                            ...
                            <div className="absolute bottom-2.5 left-2.5 z-20 bg-zinc-950/85 backdrop-blur-md px-2.5 py-1 rounded-md text-[9px] font-semibold text-zinc-300 border border-zinc-800 flex items-center gap-1.5 shadow-md">
                          ...
                          <div className="relative w-full h-full overflow-hidden bg-zinc-950">
                            ...
                            <div className="absolute bottom-2.5 left-2.5 z-20 bg-zinc-950/80 backdrop-blur-md px-2.5 py-1 rounded-md text-[9px] font-semibold text-zinc-300 border border-zinc-800 flex items-center gap-1.5">
                          ...
                          <div className="relative w-full h-full overflow-hidden bg-zinc-950 flex items-center justify-center p-2.5">
                            <img
                              src={project.imageUrl}
                              alt={project.title}
                              width="800"
                        ...
                        <div className="absolute top-2.5 left-2.5 z-20 bg-zinc-950/90 backdrop-blur-md px-2.5 py-0.5 rounded-md text-[10px] font-medium text-white font-['Montserrat'] shadow-md border border-zinc-800 flex items-center gap-1">
                          <span>{project.subCategory || project.category}</span>
                        </div>

                        {/* Number Badge */}
                        <div className="absolute top-2.5 right-2.5 z-20 bg-white/90 backdrop-blur-md px-2 py-0.5 rounded-md text-[10px] font-mono font-medium text-zinc-900 border border-zinc-200 shadow-md">
                          #{currentSlidePage * CARDS_PER_SLIDE + index + 1}
                        </div>

                        {/* Hover Overlay with Action Button */}
                        <div className="absolute inset-0 bg-zinc-950/85 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-5 text-white z-30 pointer-events-none">
                          <p className="text-zinc-200 text-[11px] mb-3 leading-relaxed font-medium line-clamp-2">
                            {project.description}
                          </p>
                          <div className="w-full bg-white text-black py-2.5 px-3 rounded-lg font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg">
                      ...
                      <div className={`p-5 flex-1 flex flex-col justify-between ${
                        isDarkBlue || isBlack ? 'bg-zinc-950 text-white' : 'bg-white text-zinc-900'
                      }`}>
                        <div>
                        ...
                        <div className={`pt-3 border-t flex items-center justify-between gap-2 ${
                          isDarkBlue || isBlack ? 'border-zinc-800' : 'border-zinc-100'
                        }`}>
          ...
          {isHomePage && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                className="px-7 py-3.5 rounded-full bg-white text-black hover:bg-zinc-200 font-bold text-xs uppercase tracking-wider flex items-center gap-2.5 transition-all shadow-xl hover:scale-105 cursor-pointer font-['Montserrat']"
              >
          ...
          <div className="mt-14 text-center p-8 rounded-3xl shadow-2xl bg-zinc-950 border border-zinc-800 text-white">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <button
                ...
                className="border-2 border-white/30 text-white hover:bg-white hover:text-black px-6 py-2.5 font-bold text-xs uppercase tracking-wider transition-all duration-300 rounded-lg cursor-pointer"
              >
                Explore Pricing
              </button>
              <button
                onClick={() => onNavigateSlide(9)}
                className="bg-white text-black hover:bg-zinc-200 px-6 py-2.5 font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-lg rounded-lg flex items-center gap-2 cursor-pointer"
              >

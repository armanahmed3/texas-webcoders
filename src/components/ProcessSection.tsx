...
export const ProcessSection: React.FC<ProcessSectionProps> = ({ onNavigateSlide, variant = 'white' }) => {
  ...
  const getStepIcon = (iconName: string) => {
    switch (iconName) {
      case 'Target':
        return <Target className={`w-5 h-5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />;
      case 'Search':
        return <Search className={`w-5 h-5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />;
      case 'Code':
        return <Code className={`w-5 h-5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />;
      case 'Rocket':
        return <Rocket className={`w-5 h-5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />;
      default:
        return <Target className={`w-5 h-5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />;
    }
  };
      ...
      >
        ...
        <div
          className={`absolute bottom-0 left-0 w-96 h-96 rounded-full blur-3xl pointer-events-none ${
            isWhite ? 'bg-zinc-200/20' : 'bg-zinc-800/20'
          }`}
        />
        ...
        <div className="relative z-10">
                ...
                return (
                  <div
                    ...
                    className={`p-6 rounded-2xl transition-all duration-300 cursor-pointer border text-center ${
                      isWhite
                        ? isActive
                          ? 'bg-zinc-950 text-white border-black shadow-xl scale-105'
                          : isPassed
                            ? 'bg-zinc-100 border-zinc-200 text-zinc-900'
                            : 'bg-zinc-50 border-zinc-200 text-zinc-700 hover:border-zinc-300'
                        : isActive
                          ? 'bg-zinc-900 border-white shadow-xl shadow-white/5 scale-105 text-white'
                          : isPassed
                            ? 'bg-zinc-900/60 border-zinc-700 text-white'
                            : 'bg-zinc-900/30 border-white/5 opacity-70 hover:opacity-100 hover:border-white/20 text-white'
                    }`}
                  >
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center text-xl font-medium mx-auto mb-4 border-2 transition-all ${
                        isWhite
                          ...
                          : isActive
                            ? 'bg-white text-black border-white shadow-lg shadow-white/20 font-medium'
                            : isPassed
                    ...
                    <h4 className={`font-semibold text-lg mb-1 font-['Montserrat'] ${
                      isWhite ? (isActive ? 'text-white' : 'text-black') : 'text-white'
                    }`}>
                      {step.title}
                    </h4>
                    <p className={`text-xs leading-relaxed ${
                      isWhite ? (isActive ? 'text-zinc-300' : 'text-zinc-600') : 'text-zinc-400'
                    }`}>
                      {step.shortDesc}
                    </p>
                  </div>
          ...
          >
            <div className={`flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b ${
              isWhite ? 'border-zinc-200' : 'border-zinc-800'
            }`}>
              ...
              <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-xs font-medium ${
                ...
                <Clock className={`w-4 h-4 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {currentStep.deliverables.map((deliv, i) => (
                  ...
                  >
                    <CheckCircle className={`w-4 h-4 flex-shrink-0 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />
                    <span>{deliv}</span>
                  </div>
                ))}
              </div>
            ...
            <div className={`mt-8 pt-6 border-t flex justify-between items-center ${
              ...
              >
                ...
                <ArrowRight className={`w-3.5 h-3.5 ${isWhite ? 'text-zinc-900' : 'text-white'}`} />
              </button>

              <button
                onClick={() => onNavigateSlide(8)}
                className="bg-white text-black px-6 py-2.5 rounded-lg font-medium text-xs uppercase tracking-wider hover:from-zinc-100 hover:to-zinc-300 transition-all cursor-pointer shadow-md shadow-white/10"
              >
                Start Project Now
              </button>
            </div>
